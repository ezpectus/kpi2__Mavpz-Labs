...
Межі сценаріїв: UC = ціль актора з обчислюваним результатом, не крок UI. Тому UC9 Authenticate - окремий спільний сценарій (auth-перевірка всередині усіх UC що вимагають логіну), а операції типу "delete comment" не виношу в окремі UC - це всередині UC5 (без функціональної декомпозиції).
